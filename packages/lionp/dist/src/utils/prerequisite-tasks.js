...
export const prerequisiteTasks = (input, pkg, options) => {
    ...
    const tasks = [
        {
            ...
            task() {
                if (pkg.publishConfig === undefined ||
                    ...
                    throw new Error('publishConfig.directory must be specified in your package.json file.');
                }
            },
        },
        {
        ...
        },
        {
            ...
            task() {
                if (!isValidInput(input)) {
                    throw new Error(`Version should be either ${getSemverIncrements().join(', ')}, or a valid semver version.`);
                }
                newVersion =
                    createVersion(pkg.version).getNewVersionFrom(input) ??
                        undefined;
