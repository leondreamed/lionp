...
export const pushGraceful = async (remoteIsOnGitHub) => {
    try {
        ...
        if (remoteIsOnGitHub && err.stderr && err.stderr.includes('GH006')) {
            ...
            return {
                pushed: 'tags',
                reason: 'Branch protection: np can`t push the commits. Push them manually.',
            };
        }
        throw error;
    }
...
export async function printCommitLog(repoUrl, registryUrl, fromLatestTag, releaseBranch) {
    ...
    if (!fromLatestTag) {
        ...
        const versionBumpCommitName = /v\d+\.\d+\.\d+/.exec(latestTag) && latestTag.slice(1); // Name v1.0.1 becomes 1.0.1
    ...
    const history = commits
        ...
        .join('\n');
    const releaseNotes = repoUrl === undefined
        ? () => ''
        : (nextTag) => commits
