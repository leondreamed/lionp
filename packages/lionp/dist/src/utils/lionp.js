...
export async function lionp(options) {
    const pkg = readPkg();
    const { version, testScript, buildScript, tests: runTests, cleanup: runCleanup, } = options;
    ...
    const isOnGitHub = options.repoUrl === undefined
        ? false
    ...
    const rollback = onetime(async () => {
        ...
        try {
            ...
            if (versionInLatestTag === readPkg().version &&
    ...
    tasks.add([
        {
            ...
            skip() {
                if (options.preview) {
                    ...
                    if (options.message) {
                        previewText += ` --message '${options.message.replace(/%s/g, version)}'`;
                    }
                    return `${previewText}.`;
                }
                return false;
        ...
        tasks.add([
            {
                ...
                async task() {
                    try {
                        ...
                        const err = error;
                        await rollback();
                        throw new Error(`Error: Build failed: ${err.message}; the project was rolled back to its previous state.`);
                    }
                },
            },
        ]);
            ...
            tasks.add([
                {
                    ...
                    task: async (context, task) => enable2fa(task, pkg.name, { otp: context.otp }),
                },
            ]);
        }
    }
