...
import { getSemverIncrements } from '~/utils/version.js';
...
test.serial('should fail when version is invalid', async () => {
	await t.throwsAsync(
		...
		{
			message: `Version should be either ${getSemverIncrements().join(
				', '
...
test.serial('should fail when version is lower as latest version', async () => {
	await t.throwsAsync(
		run(
			testedModule('0.1.0', { name: 'test', version: '1.0.0' }, { yarn: false })
		),
		{
			message:
